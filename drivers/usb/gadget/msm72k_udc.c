...
static char *default_sno = "000000000000";
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '0' || buf[0] == '1') {
		...
		if (buf[0] == '0') {
			unsigned int i;
		// A case of _safe_ programming for the future....
		for ( i = 0; (i < strlen(default_sno)) && ( i < sizeof(mfg_df_serialno)); i++)
			mfg_df_serialno[i] = default_sno[i];
			mfg_df_serialno[ i - 1 ] = '\0';
			use_mfg_serialno = 1;
